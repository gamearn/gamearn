...
// Board colours
const _darkSquare = Color(0xFF11141B);
const _lightSquare = Color(0xFFFFC21C);
...
class _PracticeDraughtsService {
  static String get _base =>
      '${ApiConfig.nodeBaseUrl}/api/v1/practice/draughts';
  ...
  Future<Map<String, dynamic>> movePiece(int fromSq, int toSq,
      {List<Map<String, dynamic>>? captures}) async {
...
class _DraughtsGameScreenState extends State<DraughtsGameScreen>
  ...
  Future<void> _executeHumanMove(int from, int to) async {
    ...
    try {
      final data = await _svc.movePiece(_serverOf(from), _serverOf(to));
      if (!mounted) return;
  ...
  /// Display the board from the local player's seat. Practice player 0 starts
  /// at the top in the server model, so rotate the board to keep "You" at the
  /// bottom. Multiplayer player 1 also needs its identities normalised.
  bool get _rotateForHuman => !_isMp || _humanIndex == 1;

  List<int> get _dispBoard {
    if (!_rotateForHuman) return _board;
    final out = List<int>.filled(_kCells, _kEmpty);
    for (int r = 0; r < _kBoardSize; r++) {
      for (int c = 0; c < _kBoardSize; c++) {
        final s = r * _kBoardSize + c;
        var cell = _board[s];
        if (_isMp && _humanIndex == 1) {
          if (cell == _kBotMan)
            cell = _kHumanMan;
          else if (cell == _kBotKing)
            cell = _kHumanKing;
          else if (cell == _kHumanMan)
            cell = _kBotMan;
          else if (cell == _kHumanKing) cell = _kBotKing;
        }
        out[(_kBoardSize - 1 - r) * _kBoardSize + (_kBoardSize - 1 - c)] = cell;
      }
    }
    return out;
  }
  ...
  int _serverOf(int dispSq) {
    if (!_rotateForHuman) return dispSq;
    final r = dispSq ~/ _kBoardSize, c = dispSq % _kBoardSize;
    return (_kBoardSize - 1 - r) * _kBoardSize + (_kBoardSize - 1 - c);
  }
    ...
    if (isKing) {
      for (final d in [
        ...
        if (nr >= 0 && nr < _kBoardSize && nc >= 0 && nc < _kBoardSize) {
          if (board[nr * _kBoardSize + nc] == _kEmpty)
            moves.add(nr * _kBoardSize + nc);
        }
      }
    } else {
  ...
  List<int> _getCapturesFrom(int from) {
    ...
    for (final d in [
      ...
      if (!isKing) {
        if (isHuman && (_rotateForHuman ? d[0] > 0 : d[0] < 0)) continue;
  ...
  void onMoveMade(String playerUid, Map<String, dynamic> move,
    ...
    if (isGameOver && !_gameOverShown) {
      _presentGameOver(isWinner: playerUid == widget.playerId, isDraw: false);
    }
  }
                    ...
                    child: Padding(
                      ...
                      child: LayoutBuilder(
                        builder: (context, constraints) {
                          // The supplied React Native artwork uses an
                          // 876 x 774 playing surface. Preserve that authored
                          // ratio instead of forcing the cells into a square.
                          const boardRatio = 876 / 774;
                          var width = constraints.maxWidth;
                          var height = width / boardRatio;
          ...
          AnimatedBuilder(
            ...
            builder: (_, __) => Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: active ? _cyan.withOpacity(_glowAnim.value) : _border,
          ...
          Row(children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: _surface,
                borderRadius: BorderRadius.circular(20.r),
                border: Border.all(color: pieces == 0 ? _orange : _border),
              ),
              child: Text('$pieces',
                  style: TextStyle(
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: const Color(0xFFFFE882), width: 2),
        boxShadow: [
          BoxShadow(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: const Color(0xFF547084), width: 2),
        ),
        child: ClipRRect(
          borderRadius: BorderRadius.circular(10.r),
...
class _PieceWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final ring = selected
        ? _selectRing
        : (isHuman ? const Color(0xFFCBD5E1) : const Color(0xFF4A2A10));
      ...
      return AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            ...
            BoxShadow(
              color: Colors.white.withOpacity(isHuman ? 0.22 : 0.06),
              blurRadius: 2,
...
class _PieceBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(children: [
        Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text('$count',
              style: TextStyle(
                  color: color, fontSize: 28.sp, fontWeight: FontWeight.w900)),
        ]),
