...
class _BrandPainter extends CustomPainter {
  ...
  void _paintGoogle(Canvas canvas, double w, double h) {
    ...
    // Red sector (top)
    canvas.drawArc(Rect.fromCircle(center: center, radius: radius), -2.8, 1.2,
        false, paint..color = const Color(0xFFEA4335));
    // Yellow sector (bottom left)
    canvas.drawArc(Rect.fromCircle(center: center, radius: radius), 1.8, 1.0,
        false, paint..color = const Color(0xFFFBBC05));
    // Green sector (bottom right)
    canvas.drawArc(Rect.fromCircle(center: center, radius: radius), 0.3, 1.5,
        false, paint..color = const Color(0xFF34A853));
    // Blue sector (right + bar)
    canvas.drawArc(Rect.fromCircle(center: center, radius: radius), -0.5, 0.8,
  ...
  void _paintFacebook(Canvas canvas, double w, double h) {
    ...
    canvas.drawRRect(
        RRect.fromRectAndRadius(
            Rect.fromLTWH(0, 0, w, h), Radius.circular(w * 0.2)),
        paint);
  ...
  void _paintApple(Canvas canvas, double w, double h) {
    ...
    final bitePath = Path()
      ..addOval(
          Rect.fromCircle(center: Offset(w * 0.85, h * 0.4), radius: w * 0.15));
