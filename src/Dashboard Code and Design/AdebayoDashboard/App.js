...
import {
  ...
  Pressable, ScrollView, StatusBar, StyleSheet, Switch, Text, TextInput,
...
import { reference, regions } from './src/art';
const { INITIAL_STATE, reducer, parseAmount, money, localDay, leaderboard } = require('./src/model');

const STORAGE_KEY = '@adebayo-dashboard/v1';
const CYAN = '#00d9ff';
const GREEN = '#12ff39';
const GAMES = [
  { id: 'ludo', name: 'Lúdò', players: '1.2k', description: 'Race your four tokens around the board and bring them home.' },
  { id: 'ayo', name: 'Ayo Ọpọ́n', players: '850', description: 'Sow seeds around the wooden board and capture your opponent’s seeds.' },
  { id: 'whot', name: 'Whot', players: '640', description: 'Match shapes or numbers and be the first to empty your hand.' },
  { id: 'draft', name: 'Draft', players: '520', description: 'Move diagonally, capture pieces, and crown your kings.' },
];

/** Reference artwork is displayed in clipped windows, without baking UI into images.
 * Replace these windows with your original PNG/WebP exports when available.
 */
function Art({ name, width, height, style }) {
  const [x, y, w, h] = regions[name];
  const factor = Math.max(width / w, height / h);
  return <View accessible={false} pointerEvents="none" style={[{ width, height, overflow: 'hidden' }, style]}>
    <Image source={reference} accessibilityIgnoresInvertColors resizeMode="stretch" style={{
      position: 'absolute', width: 590 * factor, height: 1280 * factor,
...
function Tap({ onPress, label, children, style, disabled, selected, role = 'button' }) {
  return <Pressable onPress={onPress} disabled={disabled} accessibilityRole={role}
    accessibilityLabel={label} accessibilityState={{ disabled: !!disabled, selected: !!selected }}
    hitSlop={4} style={({ pressed }) => [style, pressed && { opacity: 0.75 }, disabled && { opacity: 0.45 }]}>
    {children}
  </Pressable>;
}

function GradientButton({ title, onPress, style, compact = false, disabled = false, scale = 1 }) {
  return <Tap onPress={onPress} label={title} disabled={disabled} style={style}>
    <LinearGradient colors={['#ff9900', '#ff450d']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}
      ...
      <Text style={[ui.buttonText, compact && { fontSize: 16 * scale }]}>{title}</Text><Ionicons name="chevron-forward" size={19 * scale} color="#fff" />
    </LinearGradient>
  </Tap>;
}
...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    }).catch(() => { if (mounted.current) setStorageError('Saved data could not be loaded. This session uses demo defaults.'); })
  ...
  useEffect(() => {
    if (!ready) return;
    pendingWrites.current = pendingWrites.current.then(() => AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(state)))
      .catch(() => { if (mounted.current) setStorageError('Changes are active, but could not be saved on this device.'); });
  }, [state, ready]);

  const open = (type, data) => {
    setFormError(''); setAmount(''); setRoll(null); setDemoComplete(false);
    depositLock.current = false;
    if (type === 'profile') setName(state.name);
    if (type === 'notifications') dispatch({ type: 'READ_NOTIFICATIONS' });
    setSheet({ type, data });
  };
  const changeTab = next => { setTab(next); scroll.current?.scrollTo({ y: 0, animated: false }); };
  ...
  const addFunds = () => {
    ...
    if (parsed === null) { setFormError('Enter ₦100–₦1,000,000, using no more than two decimal places.'); return; }
  ...
  function Header() {
    return <View style={s.header}>
      <Tap label="Open your profile" onPress={() => open('profile')} style={s.member}>
        ...
        <View style={{ marginLeft: 15 * scale }}>
          {txt(state.name, 20, s.medium)}
          <View style={s.inline}><View style={s.dot} />{txt('Active Member', 15, s.cyan)}</View>
        </View>
      </Tap>
      <Tap label={state.notificationsRead ? 'Notifications' : 'Notifications, unread messages'} onPress={() => open('notifications')} style={s.roundIcon}>
        {icon('notifications-outline', 31)}{!state.notificationsRead && <View style={s.unread} />}
      </Tap>
      <Tap label="Settings" onPress={() => open('settings')} style={s.roundIcon}>{icon('settings-outline', 31)}</Tap>
    </View>;
  }

  function WalletCard() {
    return <LinearGradient colors={['#003a72', '#001324', '#00101e']} style={[s.card, s.wallet]}>
      ...
      <Tap label={`Open wallet, balance ${money(state.balance)}`} onPress={() => changeTab('Wallet')} style={{ flex: 1 }}>
        {txt('Wallet Balance', 13, s.muted)}{txt(money(state.balance), 24, s.bold)}
        {txt('+500 units this week', 13, s.cyan)}
      </Tap>
      <Tap label="Add funds to demo wallet" onPress={() => open('deposit')} style={s.plus}>{icon('add', 27, '#002741')}</Tap>
    </LinearGradient>;
  }

  function TournamentCard() {
    return <LinearGradient colors={['#172019', '#170b19', '#001b30']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.tournament}>
      ...
      <View style={s.badges}><View style={s.live}>{txt('Live Now', 13, { color: '#00110a', fontWeight: '800' })}</View>
        <View style={s.playersBadge}>{icon('people', 14, '#fff')}{txt('1,240 Players', 12)}</View></View>
      {txt('Ayo Ọpọ́n Grandmaster\nTournament', 23, s.tournamentTitle)}
      {txt('Show Your Skill. Win Big!', 15, s.muted)}
      <View style={s.prizeRow}><View>{txt('Prize Pool', 13, s.muted)}{txt('₦5,000.00', 25, [s.bold, s.cyan])}</View>
        <GradientButton title={state.joined ? 'Joined ✓' : 'Join Now'} onPress={() => open('tournament')} style={{ width: 148 * scale }} compact scale={scale} />
      </View>
    </LinearGradient>;
  }

  function GameCard({ game }) {
    return <Tap label={`${game.name}, ${game.players} playing. Open game lobby`} onPress={() => open('game', game)} style={s.gameCard}>
      <Art name={game.id} width={116 * scale} height={81 * scale} style={{ width: '100%' }} />
      <View style={s.gameText}>{txt(game.name, 15, s.bold)}
        <View style={s.inline}><View style={s.dot} />{txt(`${game.players} Playing`, 11, s.muted)}</View>
      </View>
    </Tap>;
  }

  function Streak() {
    return <LinearGradient colors={['#002332', '#07122b', '#200732']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={s.streak}>
      <View style={s.sectionRow}>{txt('Streak Progress', 18, s.bold)}{txt('Reach 30 days to unlock your reward!', 10, s.muted)}</View>
      <View style={s.milestones}>
        <View style={s.track} /><View style={[s.track, { width: `${Math.min(state.streak / 30, 1) * 32}%`, backgroundColor: CYAN }]} />
        {[7, 14, 30, 50, 90, 100].map(day => {
          const done = state.streak >= day;
          return <Tap key={day} label={`${day} day milestone, ${done ? 'completed' : 'locked'}`} onPress={() => open('streak')} style={s.milestone}>
            ...
            </View>{txt(String(day), 13, { marginTop: 6 * scale, color: day === 30 ? '#ffef00' : '#fff' })}
  ...
  function Leaderboard() {
    return <View style={{ marginTop: 15 * scale }}>
      <View style={s.sectionRow}>{txt('Global Leaderboard', 20, s.bold)}
        <View style={s.filters}>{['Daily', 'Weekly', 'Monthly', 'Yearly'].map(p =>
      ...
      {rows.map((player, index) => <Tap key={player.id} label={`${index + 1}, ${player.name}, ${player.wins} wins, ${player.xp} XP`}
        onPress={() => open('player', player)} style={[s.leaderRow, index === 0 && s.firstRow]}>
        <View style={[s.rank, index < 3 && { borderColor: ['#eab51b', '#91a6b9', '#ff8a00'][index], backgroundColor: ['#8f6400', '#526b80', '#bf4004'][index] }]}>
          {index < 3 && <View style={s.medalRibbon} />}{txt(String(index + 1), 17, index === 0 && { color: '#fff676' })}
        </View>
        <Art name={player.avatar} width={32 * scale} height={32 * scale} style={s.smallAvatar} />
        <View style={{ flex: 1 }}>{txt(player.name, 12)}{txt(`${player.wins} Wins`, 11, s.muted)}</View>
  ...
  function Home() {
    return <>
      <View style={s.welcomeRow}>
        <View style={{ flex: 1 }}>{txt('Welcome,', 24, s.bold)}
          <View style={s.inline}>{txt(state.name, 37, [s.bold, s.cyan, { maxWidth: 205 * scale }])}{txt(' 👋', 31)}</View>
          {txt('Play. Earn. Belong.', 16, s.muted)}</View>
        <Tap label="More games, bigger rewards" onPress={() => open('rewards')} style={s.promoWrap}>
          <LinearGradient colors={['#652d05', '#06152c', '#450098']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.promo}>
            ...
            {txt('More Games\nBigger Rewards', 16, [s.bold, { flex: 1, marginLeft: 10 * scale }])}{icon('chevron-forward', 25, '#c7b8ff')}
          </LinearGradient>
        </Tap>
      </View>
      <View style={s.stats}><WalletCard />
        <Tap label={`${state.streak} day streak. View details`} onPress={() => open('streak')} style={[s.card, s.streakSummary]}>
          ...
          <View style={{ flex: 1, marginLeft: 6 * scale }}><View style={s.inline}>{txt(`${state.streak} Days`, 21, s.bold)}
            <View style={s.activeBadge}>{txt('Active', 12, { color: '#001a0b', fontWeight: '700' })}{icon('checkmark-circle', 13, '#00511b')}</View></View>
            {txt('Day Streak', 14, s.muted)}{txt('Great! Keep it going.', 13, s.cyan)}</View>{icon('chevron-forward', 18, '#e6e9ff')}
        </Tap>
      </View>
      <Streak />
      <View style={s.playStrip}>{icon('information-circle', 29)}
        <View style={{ flex: 1, marginLeft: 9 * scale }}>{txt('Play a game today to keep your streak!', 13, s.medium)}
          {txt('Streak only counts if you visit and play a game successfully.', 11, [s.muted, { marginTop: 4 * scale }])}</View>
        <GradientButton title="Play Now" onPress={() => open('games')} style={{ width: 155 * scale }} compact scale={scale} />
      </View>
      <TournamentCard />
      <View style={[s.sectionRow, { marginTop: 12 * scale, marginBottom: 6 * scale }]}>{txt('Games', 20, s.bold)}
        <Tap label="See all games" onPress={() => open('games')} style={s.inline}>{txt('See All', 14, s.cyan)}{icon('chevron-forward', 17)}</Tap>
      </View>
      <View style={s.gameGrid}>{GAMES.map(game => <GameCard key={game.id} game={game} />)}</View>
      <Leaderboard />
    </>;
  }
  ...
  function OtherTab() {
    if (tab === 'Tournaments') return <>
      <Text style={ui.pageTitle}>Tournaments</Text><TournamentCard />
      <View style={ui.panel}><Text style={ui.heading}>Your registration</Text>
    ...
    if (tab === 'Wallet') return <>
      <Text style={ui.pageTitle}>Wallet</Text><View style={ui.panel}>
        <Text style={ui.body}>Available demo balance</Text><Text style={ui.balance}>{money(state.balance)}</Text>
        <GradientButton title="Add demo funds" onPress={() => open('deposit')} />
      </View><Text style={ui.heading}>Transaction history</Text>
      {state.transactions.length === 0 ? <Text style={ui.body}>No transactions yet. The opening balance is sample data.</Text> : state.transactions.map(t =>
        <View key={t.id} style={ui.transaction}><View style={{ flex: 1 }}><Text style={ui.label}>{t.label}</Text><Text style={ui.caption}>{new Date(t.date).toLocaleString()}</Text></View><Text style={ui.credit}>+{money(t.amount)}</Text></View>)}
    </>;
    return <>
      <Text style={ui.pageTitle}>Profile</Text><View style={[ui.panel, { alignItems: 'center' }]}>
        <Art name="adebayo" width={80} height={80} style={{ borderRadius: 40 }} />
        <Text style={ui.pageTitle}>{state.name}</Text><Text style={ui.credit}>● Active Member</Text>
        <Text style={ui.body}>{state.streak} day streak · 24 demo wins · 12,450 XP</Text>
      </View><GradientButton title="Edit profile" onPress={() => open('profile')} />
      <Tap label="Open settings" onPress={() => open('settings')} style={ui.listItem}><Text style={ui.label}>Settings</Text><Ionicons name="settings-outline" color={CYAN} size={24} /></Tap>
    </>;
  }
  ...
  function SheetContent() {
    switch (sheet?.type) {
      case 'notifications': return <>
        ...
        <View style={ui.notice}><Text style={ui.label}>🏆 Tournament is live</Text><Text style={ui.body}>Ayo Ọpọ́n Grandmaster Tournament is open for demo registration.</Text></View>
        <View style={ui.notice}><Text style={ui.label}>🔥 Keep your streak going</Text><Text style={ui.body}>You’re on a {state.streak} day streak. Complete today’s demo challenge to try the streak interaction.</Text></View>
        <Text style={ui.caption}>Sample notifications · All read</Text>
      </>;
      case 'settings': return <>
        <Text style={ui.heading}>Settings</Text>
        {[['sound', 'Sound effects'], ['notifications', 'Push notifications']].map(([key, label]) => <View key={key} style={ui.listItem}>
          <Text style={ui.label}>{label}</Text><Switch accessibilityLabel={label} value={state.settings[key]}
            onValueChange={value => dispatch({ type: 'SETTING', key, value })} trackColor={{ false: '#394657', true: '#008fae' }} thumbColor={state.settings[key] ? CYAN : '#cad2df'} />
        </View>)}
      ...
      case 'profile': return <>
        <Text style={ui.heading}>Edit profile</Text><Text style={ui.body}>Display name</Text>
        <TextInput accessibilityLabel="Display name" value={name} onChangeText={setName} maxLength={24} style={ui.input} placeholder="Your name" placeholderTextColor="#788b9f" autoCapitalize="words" />
        <GradientButton title="Save profile" onPress={() => { if (!name.trim()) { setFormError('Please enter your name.'); return; } dispatch({ type: 'NAME', value: name }); setSheet(null); }} />
      </>;
      case 'deposit': return <>
        <Text style={ui.heading}>Add demo funds</Text><Text style={ui.body}>This adds virtual funds on this device. It does not charge a card or make a bank transfer.</Text>
        <Text style={ui.label}>Amount (NGN)</Text>
        <TextInput accessibilityLabel="Amount in naira" value={amount} onChangeText={value => { setAmount(value); setFormError(''); }} keyboardType="decimal-pad" style={ui.input} placeholder="e.g. 1000.00" placeholderTextColor="#788b9f" maxLength={12} />
        <View style={ui.quickAmounts}>{['500', '1000', '5000'].map(value => <Tap key={value} label={`Set amount to ${value} naira`} onPress={() => setAmount(value)} style={ui.chip}><Text style={ui.label}>₦{Number(value).toLocaleString('en-US')}</Text></Tap>)}</View>
      ...
      case 'game': return <>
        ...
        <Text style={ui.body}>This is a dice challenge to test completion and streak updates, not the full {sheet.data.name} game. Roll a 4, 5, or 6 to complete it.</Text>
        ...
        <GradientButton disabled={demoComplete} title={demoComplete ? 'Completed today’s challenge' : roll ? 'Roll again' : 'Roll the dice'} onPress={() => {
          const result = 1 + Math.floor(Math.random() * 6); setRoll(result);
          if (result >= 4) { setDemoComplete(true); dispatch({ type: 'COMPLETE_DEMO', day: localDay() }); }
        }} />
      </>;
      case 'reward': return <>
        <Text style={ui.heading}>30-Day Reward</Text><Art name="gift" width={90} height={105} style={{ alignSelf: 'center' }} />
      ...
      case 'rewards': return <>
        ...
        <Tap label="View 30 day reward" onPress={() => open('reward')} style={ui.listItem}><Text style={ui.label}>View 30-Day Reward</Text><Ionicons name="gift-outline" size={24} color={CYAN} /></Tap>
      </>;
      case 'player': return <>
        <Art name={sheet.data.avatar} width={72} height={72} style={{ borderRadius: 36, alignSelf: 'center' }} />
        <Text style={ui.heading}>{sheet.data.name}</Text><Text style={ui.body}>{period} leaderboard{ '\n' }{sheet.data.wins} Wins{ '\n' }{sheet.data.xp.toLocaleString('en-US')} XP</Text>
  ...
  if (!ready) return <View style={ui.loading}><StatusBar barStyle="light-content" backgroundColor="#00101e" /><ActivityIndicator color={CYAN} size="large" accessibilityLabel="Loading dashboard" /></View>;

  return <View style={ui.root}>
    <StatusBar barStyle="light-content" backgroundColor="#00101e" />
    <LinearGradient colors={['#001526', '#000e19', '#00101b']} style={StyleSheet.absoluteFillObject} />
    <ScrollView ref={scroll} showsVerticalScrollIndicator={false} contentContainerStyle={[s.content, { paddingTop: insets.top + 10 * scale, paddingBottom: 28 * scale }]}>
      <Header />
      {!!storageError && <Text accessibilityLiveRegion="polite" style={ui.error}>{storageError}</Text>}
      {tab === 'Home' ? <Home /> : <OtherTab />}
    </ScrollView>
    <View style={[s.bottomBar, { paddingBottom: Math.max(insets.bottom, 14 * scale), paddingLeft: insets.left, paddingRight: insets.right }]}>
      {[
        ['Home', 'home', 'home-outline'], ['Tournaments', 'trophy', 'trophy-outline'],
        ['Wallet', 'wallet', 'wallet-outline'], ['Profile', 'person', 'person-outline'],
      ].map(([label, active, inactive]) => <Tap key={label} label={label} role="tab" selected={tab === label} onPress={() => changeTab(label)} style={s.navItem}>
        <Ionicons name={tab === label ? active : inactive} size={31 * scale} color={tab === label ? CYAN : '#f8f8ff'} style={tab === label && s.navGlow} />
        {txt(label, 15, { color: tab === label ? CYAN : '#f8f8ff', marginTop: 5 * scale })}
      </Tap>)}
    </View>
    <Modal transparent visible={sheet !== null} animationType="slide" onRequestClose={() => setSheet(null)} statusBarTranslucent>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={ui.modalBackdrop}>
        ...
        <View accessibilityViewIsModal style={[ui.sheet, { paddingBottom: Math.max(insets.bottom, 20), maxHeight: '88%' }]}>
          <View style={ui.sheetTop}><View style={ui.handle} /><Tap label="Close dialog" onPress={() => setSheet(null)} style={ui.close}><Ionicons name="close" size={26} color="#fff" /></Tap></View>
...
// Layout measurements use the reference's 590px coordinate system. They scale
// with the available width and cap at 650px on tablets. Dialogs use normal dp.
function makeStyles(k) {
  const base = {
    content: { width: '100%', maxWidth: 650, alignSelf: 'center', paddingHorizontal: 30 },
    text: { color: '#f7f7ff', fontSize: 18 }, bold: { fontWeight: '700' }, medium: { fontWeight: '500' },
    cyan: { color: CYAN }, muted: { color: '#c4c7dd' }, inline: { flexDirection: 'row', alignItems: 'center' },
    ...
    avatar: { borderRadius: 30 }, roundIcon: { width: 56, height: 56, borderRadius: 28, backgroundColor: '#002138', alignItems: 'center', justifyContent: 'center' },
    ...
    promoWrap: { width: 241, borderRadius: 17, borderColor: '#83581f', borderWidth: 1, overflow: 'hidden' },
    promo: { flexDirection: 'row', alignItems: 'center', height: 78, paddingHorizontal: 10 },
    stats: { flexDirection: 'row', gap: 10, marginBottom: 9 },
    card: { flex: 1, height: 85, borderWidth: 1, borderColor: '#086d96', borderRadius: 12, flexDirection: 'row', alignItems: 'center' },
    wallet: { paddingHorizontal: 10 }, walletIcon: { width: 52, height: 56, backgroundColor: '#003155', borderRadius: 25, alignItems: 'center', justifyContent: 'center', marginRight: 10 },
    plus: { width: 35, height: 35, borderRadius: 18, backgroundColor: '#00baf2', alignItems: 'center', justifyContent: 'center', marginLeft: 5 },
    streakSummary: { paddingHorizontal: 8, backgroundColor: '#001522' },
    ...
    streak: { borderRadius: 12, borderWidth: 1, borderColor: '#087f94', paddingHorizontal: 16, paddingTop: 7, height: 122 },
    sectionRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
    milestones: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 11, alignItems: 'flex-start', flex: 1 },
    track: { position: 'absolute', top: 16, left: 17, right: 78, height: 4, backgroundColor: '#3c455e' },
    milestone: { width: 37, alignItems: 'center' },
    milestoneCircle: { width: 33, height: 33, borderRadius: 17, borderWidth: 2, borderColor: '#64778f', backgroundColor: '#142035', alignItems: 'center', justifyContent: 'center' },
    ...
    reward: { width: 79, height: 68, borderRadius: 9, borderWidth: 1, borderColor: '#946d39', alignItems: 'center', justifyContent: 'flex-end', paddingBottom: 3, backgroundColor: '#07162b', marginTop: 5 },
    gift: { position: 'absolute', top: -34 },
    playStrip: { marginTop: 9, marginBottom: 9, height: 49, paddingHorizontal: 8, flexDirection: 'row', alignItems: 'center', borderRadius: 10, backgroundColor: '#09152f' },
    tournament: { height: 187, borderRadius: 13, borderWidth: 1, borderColor: '#ac7427', overflow: 'hidden', paddingHorizontal: 21, paddingVertical: 9 },
    ...
    playersBadge: { backgroundColor: '#162131', flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 18 },
    tournamentTitle: { fontWeight: '700', lineHeight: 24, marginBottom: 5, width: 300 },
    ...
    gameCard: { flex: 1, height: 130, borderRadius: 10, borderWidth: 1, borderColor: '#095674', overflow: 'hidden', backgroundColor: '#021626' },
    gameText: { paddingHorizontal: 9, paddingTop: 4, gap: 4 },
    filters: { width: 282, height: 29, flexDirection: 'row', borderRadius: 12, backgroundColor: '#05152c', borderWidth: 1, borderColor: '#132842', overflow: 'hidden' },
    filter: { flex: 1, alignItems: 'center', justifyContent: 'center', borderRadius: 11 },
    activeFilter: { backgroundColor: '#00bdf3' },
    leaderRow: { height: 41, marginTop: 4, borderRadius: 10, borderWidth: 1, borderColor: '#1b2b40', backgroundColor: '#031626', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12 },
    firstRow: { borderColor: '#b49125', backgroundColor: '#161a1b' },
...
const ui = StyleSheet.create({
  root: { flex: 1, backgroundColor: '#00101e' }, loading: { flex: 1, backgroundColor: '#00101e', justifyContent: 'center', alignItems: 'center' },
